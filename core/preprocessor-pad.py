...
def resize_image_random_method(image, target_size):
  ...
  with tf.name_scope('ResizeRandomMethod', values=[image]):
    ...
    resized_image = _apply_with_random_selector(
        image,
        lambda x, method: tf.image.resize_image_with_pad(x, target_size[0], target_size[1], method),
        num_cases=4)
...
def resize_image(image,
  ...
  with tf.name_scope('ResizeImage', values=[image, target_size]):
    new_image = tf.image.resize_image_with_pad(image, target_size[0], target_size[1],
                                       method=method)
  return new_image
